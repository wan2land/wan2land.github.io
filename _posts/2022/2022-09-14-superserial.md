...
실제 내부에 파서 만드는 과정은 복잡해서 생략하였는데 더 자세한 과정이 궁금하시면, 과거에 파서에 관한 글([파서 만들기 (1) - JSON.stringify](https://wan2.land/posts/2020/02/11/make-parser-1/))을 한번 읽어보시는 걸 추천드립니다. 파서에서는 어떻게 표현하게 만드는지가 중요하지, 만드는 방법 자체는 크게 다르지 않기 때문입니다.
